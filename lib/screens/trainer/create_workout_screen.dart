...
import 'package:gym_management_app/blocs/workout/workout_event.dart';
import 'package:gym_management_app/blocs/workout/workout_state.dart';
      ...
      body: BlocConsumer<WorkoutBloc, WorkoutState>(
        ...
        builder: (context, state) {
          return state.maybeWhen(
            ...
            exercisesLoaded: (exercises) => _buildBody(exercises),
            orElse: () {
              // If it's already loaded but something else happened, we still want to show the form
              // However, Bloc version might be tricky here.
              // For simplicity, let's assume exercisesLoaded is the state we stay in.
  ...
  Widget _buildBody(List<WorkoutExerciseModel> exercises) {
    ...
    if (widget.memberId == null) {
      return BlocBuilder<MemberBloc, MemberState>(
        builder: (context, state) {
          return state.maybeWhen(
            membersLoaded: (members) => _buildForm(exercises, members),
            loading: () => const Center(child: CircularProgressIndicator()),
